...
class PHPRestSQL {
    ...
    function PHPRestSQL($iniFile = 'phprestsql.ini') {
        ...
        if (isset($_SERVER['REQUEST_URI']) && isset($_SERVER['REQUEST_METHOD'])) {
            ...
            {
                $param = substr($lastPart, strpos($lastPart, "?") + 1);
                $this->params = explode("&", $param);
                for ($i = 0; $i < count($this->params); $i++)
                {
                    $this->params[$i] = str_replace("%22", "\"", $this->params[$i]);
                    $this->params[$i] = str_replace("%20", " ", $this->params[$i]);
                    $this->params[$i] = str_replace("%3E", ">", $this->params[$i]);
                    $this->params[$i] = str_replace("%3C", "<", $this->params[$i]);
                }
                $lastPart = substr($lastPart, 0, -(strlen($param) + 1));
            }
        ...
        if ($this->table) {
            ...
            if ($primary) {
                if ($this->uid && count($primary) == count($this->uid)) { // get a row
                    ...
                    $this->display = 'table';
                    $where = '';
                    foreach ($this->params as $cond) {
                        $where .= $cond.' AND ';
                    }
                    $where = substr($where, 0, -5);
                    $resource = $this->db->getTableRows(join(', ', $primary), $this->table, $where);
                    if ($resource) {
                        if ($this->db->numRows($resource) > 0) {
                            while ($row = $this->db->row($resource)) {
